...
(def ^:const INTERVAL_5_MIN_IN_MS (* 5 60 1000))
(def ^:const TEXT_5_MIN "5 min")
(def ^:const INTERVAL_15_MIN_IN_MS (* 15 60 1000))
(def ^:const TEXT_15_MIN "15 min")
(def ^:const INTERVAL_30_MIN_IN_MS (* 30 60 1000))
(def ^:const TEXT_30_MIN "30 min")
(def ^:const INTERVAL_45_MIN_IN_MS (* 45 60 1000))
(def ^:const TEXT_45_MIN "45 min")
(def ^:const INTERVAL_1_HOUR_IN_MS (* 60 60 1000))
...
(defn trigger-mute-interval [activity interval_ms text]
  ...
  (let [alarm-manager (get-service :alarm)
    ...
    (.set alarm-manager
          ...
          (+ interval_ms (android.os.SystemClock/elapsedRealtime))
          pi)
    (reset! pending-intent pi)
    (.add calendar java.util.Calendar/MILLISECOND interval_ms)
...
(defn main-layout [activity]
  [:linear-layout {:orientation :vertical
   ...
   [:button {:text TEXT_5_MIN,
             :on-click (fn [_] (trigger-mute-interval activity INTERVAL_5_MIN_IN_MS TEXT_5_MIN))}]
   [:button {:text TEXT_15_MIN,
             :on-click (fn [_] (trigger-mute-interval activity INTERVAL_15_MIN_IN_MS TEXT_15_MIN))}]
   [:button {:text TEXT_30_MIN,
             :on-click (fn [_] (trigger-mute-interval activity INTERVAL_30_MIN_IN_MS TEXT_30_MIN))}]
   [:button {:text TEXT_45_MIN,
             :on-click (fn [_] (trigger-mute-interval activity INTERVAL_45_MIN_IN_MS TEXT_45_MIN))}]
